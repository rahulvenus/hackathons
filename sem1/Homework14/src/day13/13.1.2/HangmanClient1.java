...
public class HangmanClient1 {
	...
	public static void main(String[] args) {
		if(args.length == 1){
			...
			//registryNum = Integer.parseInt(args[1]);	
			try {
				playGame();
			} catch (IOException e) {
			...
			System.out.println("Correct Usage : HangmanClient1 <serveraddress>");
		}
	...
	private static void playGame() throws IOException {
        ...
        try 
        {
	   // Registry registry = LocateRegistry.getRegistry(registryNum);
           PlayTheGame client = (PlayTheGame) Naming.lookup( "//" + 
        		   serverAddress + 
                "/client1");         //objectname in registry 
           System.out.println("reached");
