 ...
 * 
 * @version: $Id: CalculatorModel.java,v 1.70 2015/10/05 12:00:00 
 * 
...
/* The Model Class contains all the data and methods
...
public class CalculatorModel {
	...
	public double calcExpression(String calcExpression) {
		...
		index++) {
			...
			if ((aChar == "(") || (aChar == ")")) {
				if (!(temp.isEmpty())) expression.addElement(temp);
				...
				if (operatorStack.isEmpty() && (ignorePrecedence == 0)) {
					...
					if (ignorePrecedence == 0) {
						while (!operatorStack.isEmpty()) {
							...
							if (operatorPreced1(operInStack.charAt(0)) > operatorPreced1(token.charAt(0))) {
...
class MyStringStack {
	...
	public void push(String S) {
		...
		if (top < size) {
			elements[top] = S;
		} else allocMoreSpace(S); //allocate more space if needed
